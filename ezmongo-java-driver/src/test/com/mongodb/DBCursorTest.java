...
public class DBCursorTest extends TestCase {
    ...
    public void testExplain() {
        ...
        if (!Mongo.isClientModeEnabled()) { assertEquals(20, c.find(q).limit(-20).itcount()); }
        ...
        if (!Mongo.isClientModeEnabled()) {assertEquals(20, c.find(q).limit(-20).explain().get("n")); }
    ...
    public void testSpecial() {
        ...
        if (!Mongo.isClientModeEnabled()) { // TODO returnKey only return the index fields, need to find a way to do this with aggregate
    ...
    public void testUpsert() {
        ...
        if (Mongo.isClientModeEnabled()) {
            try {
                ...
                assertTrue(e.getMessage().contains("upsert is not supported"));
            }
        } else {
            c.update(new BasicDBObject("page", "/"), new BasicDBObject("$inc", new BasicDBObject("count", 1)), true, false);
            c.update(new BasicDBObject("page", "/"), new BasicDBObject("$inc", new BasicDBObject("count", 1)), true, false);
    ...
    public void testComment() {
        // Mongo.isClientModeEnabled() - redact has no commenting capability.
        // we wouldn't allow this tenant apps doesn't get access to query profile logs. But this test case will pass because
        // the query is still executed, redacted of course
        DBCursor cursor = new DBCursor(collection, new BasicDBObject(), new BasicDBObject(), ReadPreference.primary())
