...
public class DBRefTest extends TestCase {
    ...
    public void testRoundTrip(){
        ...
        if (Mongo.isClientModeEnabled()) {
            a.insert(BasicDBObjectBuilder.start("_id", 17).add("n", 111).get());
            b.insert(BasicDBObjectBuilder.start("n", 12).add("l", new DBRef(getDatabase(), a.getName(), 17)).get());
        } else {
    ...
    public void testFindByDBRef(){
        ...
        if (!Mongo.isClientModeEnabled()) {  // we make collection transient so _db in fetch will be null
