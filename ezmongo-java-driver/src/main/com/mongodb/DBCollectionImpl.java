...
public class DBCollectionImpl extends DBCollection implements Serializable {
    protected final DBApiLayer db;
    protected final String namespace;

    public DBCollectionImpl(final DBApiLayer db, String name){
        super(db, name );
    ...
    @Override
    public QueryResultIterator find(DBObject ref, DBObject fields, int numToSkip, int batchSize, int limit, int options,
    ...
    public Cursor aggregateWithOut(final List<DBObject> pipeline, final AggregationOptions options,
                            final ReadPreference readPreference) {
        ...
        if (outCollection != null) {
            Integer batchSize = options.getBatchSize();
            qri = new QueryResultIterator(res, db, this, batchSize == null ? 0 : batchSize, getDecoder(), res.getServerUsed());
        }
        return qri;
    }
