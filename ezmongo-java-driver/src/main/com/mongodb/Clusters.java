...
final class Clusters {
    ...
    public static Cluster create(final String clusterId, final ClusterSettings settings, final ServerSettings serverSettings,
        ...
        ClusterableServerFactory serverFactory = new DefaultClusterableServerFactory(clusterId, serverSettings, scheduledExecutorService,
                                                                                     mongo);
        if (Mongo.isClientModeEnabled() && settings.getMode() == ClusterConnectionMode.ClientMode) { // yes redundant but on purpose
            return new ClientModeServerCluster(clusterId, settings, serverFactory,
                    clusterListener != null ? clusterListener : new NoOpClusterListener());
        } else if (settings.getMode() == ClusterConnectionMode.Single) {
