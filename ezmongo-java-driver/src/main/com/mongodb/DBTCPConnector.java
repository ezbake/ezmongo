 ...
 */
@Deprecated
public class DBTCPConnector implements DBConnector, Serializable {
    ...
    public DBTCPConnector( Mongo mongo  ) {
        ...
        if (Mongo.isClientModeEnabled()){
            connectionMode = ClusterConnectionMode.ClientMode;
        } else {
    ...
    public Response call( DB db, DBCollection coll, OutMessage m, ServerAddress hostNeeded, int retries,
        ...
        if (Mongo.isClientModeEnabled()){
            return innerCallToThrift(db, coll, m, hostNeeded, retries, readPref, decoder);
        } else {
            isTrue("open", !_closed);
            try {
    ...
    private Response innerCallToThrift(final DB db, final DBCollection coll, final OutMessage m, final ServerAddress hostNeeded,
                               final int remainingRetries, ReadPreference readPref, final DBDecoder decoder) {
        if (coll instanceof EzDBCollectionImpl){
           EzDBCollectionImpl d = (EzDBCollectionImpl)coll;
           Response r = d.getMore(hashCode(), m,remainingRetries,readPref,decoder);
           return r;
        } else {
