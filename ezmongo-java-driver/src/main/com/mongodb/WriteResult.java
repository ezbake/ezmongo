...
public class WriteResult implements Serializable {
    ...
    public synchronized CommandResult getLastError(WriteConcern concern){
        if (Mongo.isClientModeEnabled()){
            ...
            try {
                CommandResult c = new CommandResult(new ServerAddress("localhost"));
                c.put("ok",new Boolean(true));
                return c;
            } catch (UnknownHostException e) {
                throw new IllegalStateException("Can't new up fake ServerAddress");
            }
        } else {
            if (_lastErrorResult != null) {
                // do we have a satisfying concern?
    ...
    transient final private DB _db;
    transient final private DBPort _port;
